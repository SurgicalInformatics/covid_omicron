...
nims = read_csv("/home/common/covid/nims/COCIN_NIMS__20220324.csv") %>% 
  rename_with(tolower)

voc = read_csv("/home/common/covid/nims/COCIN_VOC_Lineage__20220126T162002Z.csv") %>% 
  rename_with(tolower)

sgtf = read_csv("/home/common/covid/nims/COCIN_SGTF__20220324.csv") %>% 
  rename_with(tolower)
...
source("https://raw.githubusercontent.com/SurgicalInformatics/cocin_ccp/master/02_functions.R")
```
...
tmp = oneline %>% 
  ...
  mutate(
    symptoms_all_missing = across(fever_ceoccur_v2:anosmia_ceoccur_v2) %>%
      {!is.na(.)} %>%
      rowSums(),
    symptoms_all_missing = if_else(symptoms_all_missing == 0, "Yes", "No"), 
    
    number_symptoms = across(fever_ceoccur_v2:anosmia_ceoccur_v2) %>% 
      {. == "Yes"} %>% 
      rowSums(na.rm = TRUE) %>% 
      if_else(symptoms_all_missing == "Yes", NA_real_, .) %>% 
...
vac = oneline %>% 
  ...
  mutate(vac_at_admission = if_else(is.na(vac_at_admission) & 
         ...
         # 18/04/2022 consider unknown to be not vaccinated. 
...
oneline = oneline %>% 
  ...
  mutate(epoch = case_when(
    ...
    TRUE ~ "14 Dec 2021 - 26 Mar 2022") %>% 
      fct_relevel("Jan 2020 - Aug 2020", "Sept 2020 - April 2021",  "May 2021 - 13 Dec 2021", "14 Dec 2021 - 26 Mar 2022") %>% 
...
```{r}
out = oneline %>%
  summary_factorlist("isoweek_date", c("vac_at_admission", "vac_n_gr28", "vac_n_gr28_factor"))
```
...
# Model structure
dependent = "Surv(time, status)"
explanatory = c("epoch", "vac_n_gr28_factor", "age.factor", "sex", "imd_quintile",
                        "ethnicity_4levels",
                        "number_comorbid", 
                        "number_symptoms")# , "isaric_phys")
  explanatory = c("epoch_vac_age", "sex", "imd_quintile",
                          "ethnicity_4levels",
                          "number_comorbid")#                         "number_symptoms")# , "isaric_phys")
tmp = oneline_reduced %>% 
  mutate(
    epoch_vac_age = fct_cross(epoch, vac_n_gr28_factor, age.factor, sep = "_")
  ) %>% 
  finalfit(dependent, explanatory, condense = FALSE)
  

tmp %>% 
  separate(2, sep = "_", into = c("epoch", "vac_n_gr28_factor", "age.factor"))	%>% 
  select(epoch, vac_n_gr28_factor, age.factor, HR = HR..multivariable., L95 = L95.y, U95 = U95.y) %>% 
  drop_na(vac_n_gr28_factor) %>% 
  mutate(HR = as.numeric(HR),
         ...
         wave = factor(epoch) %>% 
           fct_recode(
             "1" = "Jan 2020 - Aug 2020", 
             "2" = "Sept 2020 - April 2021", 
             "3" = "May 2021 - 13 Dec 2021",
             "Omicron" = "14 Dec 2021 - 26 Mar 2022"
           ) %>% 
           fct_relevel("1", "2", "3", "4")
         ) %>% 
  ggplot_lancet(aes(x = wave, y = HR, ymin = L95, ymax = U95, group = vac_n_gr28_factor, colour = vac_n_gr28_factor)) +
  geom_pointrange() +
  geom_line() +
  scale_color_brewer("Vaccination", palette = "Set1") +
  ylim(c(0, 22)) +
  labs(x = "Wave", y = "Hazard ratio for death") +
  facet_wrap(. ~ age.factor) +
  ggtitle("Risk of death in hospitalised patients with COVID-19",
          "Hazard ratio with 95% confidence interval, adjusted for sex, deprivation, ethnicity, comorbidity")
  theme_bw()

```

```{r}
master_complications_list = c("cryptogenic_ceterm", "ards_ceoccur", "pneumothorax_ceterm", 
                              "pleuraleff_ceterm", "bactpneu_ceoccur",
                              "meningitis_ceterm", "seizure_ceterm", "stroke_ceterm", "neuro_comp", 
                              "heartfailure_ceterm", "endocarditis_aeterm", 
                              "myocarditis_ceterm", "cardiomyopathy_ceterm", "arrhythmia_ceterm", "ischaemia_ceterm", 
                              "cardiacarrest_ceterm",  
                              "bacteraemia_ceterm", "coagulo_ceterm", "aneamia_ceterm",  "rhabdomyolsis_ceterm", 
                              "renalinjury_ceterm", "gastro_ceterm", "pancreat_ceterm",
                              "dvt_ceterm", "pulmthromb_ceterm", 
                              "liverdysfunction_ceterm")
oneline = oneline %>% 
  mutate(comp = select(., master_complications_list) %>% 
           {. == "YES"} %>% 
           rowSums(na.rm = TRUE) %>% 
           ff_label("Number of complications")
  ) %>% 
  mutate(comp_factor = case_when(
    comp == 0 ~ "0",
    comp == 1 ~ "1",
    comp > 1 ~ "2+"
  )
  ) %>% 
  mutate(comp_yn = ifelse(comp == 0, "No", "Yes")) %>%
  mutate(comp_status = ifelse(comp == 0, 0, 1))
```
  ...
  mutate(
    wave = factor(epoch) %>% 
      fct_recode(
        ...
        "2" = "Sept 2020 - April 2021", 
        "3" = "May 2021 - 13 Dec 2021",
        "4" = "14 Dec 2021 - 26 Mar 2022"
      ) %>% 
      fct_relevel("1", "2", "3", "4")
  ) %>% 
  ggplot_lancet(aes(x = anydat.day1, y = comp_status, colour = vac_n_gr28_factor)) + 
  geom_smooth(se = FALSE) + 
  scale_x_date(date_breaks = "1 year", date_labels = "%b %Y") +
  scale_y_continuous(label = scales::percent_format()) +
  scale_color_brewer("Vaccination", palette = "Set1") +
  labs(x = "Admission date", y = "Complication (%)") +
  facet_grid(number_comorbid.factor ~ age.factor) + 
  ggtitle("Complication rates over time stratified by age (horizontal), number comorbidities (vertical) and vaccination status") + 
...
oneline_reduced = oneline_reduced %>% 
  mutate(
    epoch = fct_relevel(epoch, "Sept 2020 - April 2021")
  )

dependent = "Surv(time, status)"
oneline_reduced %>% 
  coxphmulti(dependent, explanatory_full)

oneline_reduced %>% 
  recipe(status + time ~ epoch + age.factor + sex + imd_quintile + vac_n_gr28 + ethnicity_4levels + 
           number_comorbid + number_symptoms) %>% 
  step_dummy(all_nominal()) %>% 
  prep() %>% 
  juice() %>%
  regmedint( yvar = "time",
             avar = 
               # "epoch_May.2021...13.Dec.2021", 
             "epoch_X14.Dec.2021...26.Mar.2022",
             mvar = "vac_n_gr28",
             cvar = c(# "epoch_Sept.2020...April.2021", 
               "epoch_Jan.2020...Aug.2020", 
               # "epoch_X14.Dec.2021...26.Mar.2022", 
               "epoch_May.2021...13.Dec.2021",
               "age.factor_X50.69", "age.factor_X70.79",
               "age.factor_X80.", 
               "sex_Female", 
               "imd_quintile_X2", "imd_quintile_X3", "imd_quintile_X4", 
               "imd_quintile_X5...least.deprived", "imd_quintile_Unknown", 
               "ethnicity_4levels_South.Asian", "ethnicity_4levels_East.Asian", 
               "ethnicity_4levels_Black", "ethnicity_4levels_Other.Ethnic.Minority", 
               "ethnicity_4levels_Unknown"),
             eventvar = "status",
...
# 27.3% wave 4 vs wave 2
# 19.5% wave 3 vs wave 2
```
...
```{r eval=FALSE, include=FALSE}
# Contrasts
# "Jan 2020 - Aug 2020"       "Sept 2020 - April 2021"    "May 2021 - 13 Dec 2021"    "14 Dec 2021 - 26 Mar 2022"
...
summary(neMod1)
effdecomp = neEffdecomp(neMod1, xRef = c("Sept 2020 - April 2021", "14 Dec 2021 - 26 Mar 2022"))
summary(effdecomp)
...
neMod2 = neModel(any_oxygen ~ epoch0 + epoch1 + 
                 ...
                 expData = expData, se = "robust")
summary(neMod2)
effdecomp2 = neEffdecomp(neMod2, xRef = c("Sept 2020 - April 2021", "14 Dec 2021 - 26 Mar 2022"))
summary(effdecomp2)
...
summary(neMod3)
effdecomp3 = neEffdecomp(neMod3, xRef = c("Sept 2020 - April 2021", "14 Dec 2021 - 26 Mar 2022"))
summary(effdecomp3)
...
neMod2 = neModel(any_icu ~ epoch0 + epoch1 + 
                 ...
                 expData = expData, se = "robust")
summary(neMod2)
effdecomp2 = neEffdecomp(neMod2, xRef = c( "May 2021 - 13 Dec 2021" , "14 Dec 2021 - 26 Mar 2022"))
summary(effdecomp2)
regmedint:::prop_med_yreg_logistic(coef(effdecomp2)[["natural direct effect"]], 
                                   coef(effdecomp2)[["natural indirect effect"]])
...
effdecomp1m =  neMod1m %>% 
  map(~ neEffdecomp(., xRef = c("Sept 2020 - April 2021", "14 Dec 2021 - 26 Mar 2022"))
  )
...
effdecomp2 = neEffdecomp(neMod_mi , xRef = c("Sept 2020 - April 2021", "14 Dec 2021 - 26 Mar 2022"))
data.frame(
...
```{r}
oneline %>% 
  filter(any_oxygen == "Yes") %>% 
  mutate(
    any_steroid = as.numeric(any_steroid) - 1, 
    anydat.month = anydat.day1 %>% format("%Y-%m") %>% paste0("-01") %>% ymd()
  ) %>% 
  filter(anydat.day1 > ymd(20200201)) %>% 
  filter(anydat.day1 < ymd(20220301)) %>% 
  group_by(anydat.month) %>% 
  summarise(
    n = n(),
    any_steroid = sum(any_steroid, na.rm = TRUE),
    any_steroid_prop = any_steroid / n,
  ) %>% 
  ggplot_lancet(aes(x = anydat.month, y = any_steroid_prop)) +
  geom_line(colour = "darkblue") + 
  geom_hline(yintercept = 0.85, linetype = 2, colour = "red") +
  scale_y_continuous(label = scales::percent_format()) +
  annotate("text", x = ymd(20200201), y = 0.82, label = "85%", colour = "red") +
  labs(x = "Admission date", y = "Proportion of patients receiving any corticosteroid (%)") +
  ggtitle("Proportion of hospitalised COVID-19 patients with an indication for steroid who received it")
